	...
	for woocommerce_order in get_woocommerce_orders():
		if valid_customer_and_product(woocommerce_order):
			try:
				...
				if e.args and e.args[0] and str(e.args[0]).decode("utf-8").startswith("402"):
					raise e
				else:
...
def valid_customer_and_product(woocommerce_order):
	...
	if customer_id == 0: # we are dealing with a guest customer 
		...
		if not woocommerce_settings.default_customer:
			make_woocommerce_log(title="Missing Default Customer", status="Error", method="valid_customer_and_product", message="Missing Default Customer in Woocommerce Sync Settings",
				request_data=woocommerce_order, exception=True)
			return False
	...
	warehouse = frappe.get_doc("Woocommerce Sync Settings", "Woocommerce Sync Settings").warehouse
	for item in woocommerce_order.get("line_items"):
		if item.get("product_id") and not frappe.db.get_value("Item", {"woocommerce_sync_product_id": item.get("product_id")}, "name"):
			try:
				item = get_request("products/{0}".format(item.get("product_id")))["product"]
				make_item(warehouse, item, woocommerce_item_list=[])
			except Exception as e:
				make_woocommerce_log(title="Get Item", status="Error", method="valid_customer_and_product", message="Get item from woo error",
				request_data=item.get("product_id"), exception=True)
...
def get_item_code(woocommerce_item):
	item_code = frappe.db.get_value("Item", {"woocommerce_sync_variant_id": woocommerce_item.get("variation_id")}, "item_code")
	if not item_code:
		item_code = frappe.db.get_value("Item", {"woocommerce_sync_product_id": woocommerce_item.get("product_id")}, "item_code")
	#if not item_code:
...
def get_shipping_account_head(shipping):
        ...
        shipping_account =  frappe.db.get_value("Woocommerce Tax Account", \
                {"parent": "Woocommerce Sync Settings", "woocommerce_tax": shipping_title}, "tax_account")
...
def get_tax_account_head(tax):
	...
	tax_account =  frappe.db.get_value("woocommerce Tax Account", \
		{"parent": "Woocommerce Sync Settings", "woocommerce_tax": tax_title}, "tax_account")
